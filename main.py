...
def prompter():
    files = "22046531-TheIndianPortsAct-87257385.pdf"
    ...
    while not matches and retries < max_retries:
        if retries > 0:
            print(f"⚠️ No Q&A pairs found for {files}. Retrying ({retries}/{max_retries})")
        ...
        **Requirements:**
        ...
        - Dont Miss the Main Conditions like 20-25 Q&A Pairs, Thorough Analysis, Strict Format, No Repetition, Context Awareness, Document Focus.
    ...
    if matches:
        output_file = files.replace(" ", "_").replace(".pdf","")
        ...
        print(f"❌ Failed to generate Q&A pairs for {files} after {max_retries} retries. Skipping.")
